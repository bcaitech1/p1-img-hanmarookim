...
class MaskBaseDataset(data.Dataset):
    ...
    def set_val_ratio(self, val_ratio):
        self.val_ratio = val_ratio

    def __getitem__(self, index):
        image = self.read_image_custom(index)
    ...
    def read_image_custom(self, index):
        image_path = self.image_paths[index]
...
class MaskSplitByProfileDataset58(MaskSplitByProfileDataset):
    """
        train / val 나누는 기준을 이미지에 대해서 random 이 아닌
        사람(profile)을 기준으로 나눕니다.
        구현은 val_ratio 에 맞게 train / val 나누는 것을 이미지 전체가 아닌 사람(profile)에 대해서 진행하여 indexing 을 합니다
        ...
        for phase, indices in split_profiles.items():
            for _idx in indices:
                ...
                for file_name in os.listdir(img_folder):
                    ...
                    id, gender, race, age = profile.split("_")
                    gender_label = getattr(self.GenderLabels, gender)
                    age_label = self.AgeGroup58.map_label(age)
...
class MultiPredictMaskBaseDataset(data.Dataset):
    num_classes = 3 * 2 * 3
    class MaskLabels:
        mask = 0
        incorrect = 1
...
class MultiPredictMaskSplitByProfileDataset(MultiPredictMaskBaseDataset):
    """
        train / val 나누는 기준을 이미지에 대해서 random 이 아닌
        사람(profile)을 기준으로 나눕니다.
        구현은 val_ratio 에 맞게 train / val 나누는 것을 이미지 전체가 아닌 사람(profile)에 대해서 진행하여 indexing 을 합니다
